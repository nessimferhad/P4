...
class Article extends Model
{
    ...
    {
        $query = $this->pdo->prepare('UPDATE articles SET title = :title, introduction = :introduction, content = :content, created_at = NOW() WHERE `id` = :id');
        $query->execute(compact('title', 'introduction', 'content', 'created_at', 'id'));
    }
}
